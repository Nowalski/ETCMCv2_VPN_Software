...
# Script to install the ETCMCv2 VPN Server (Shadowbox) docker container, a watchtower docker container
# (to automatically update the server), and to create a new ETCMCv2 VPN access key.

# You may set the following environment variables, overriding their defaults:
# SB_IMAGE: The VPN Server Docker image to install (default: quay.io/outline/shadowbox:stable)
...
# SHADOWBOX_DIR: Directory for persistent ETCMCv2 VPN Server state.
...
FULL_LOG="$(mktemp -t etcmc_logXXXXXXXXXX)"
LAST_ERROR="$(mktemp -t etcmc_last_errorXXXXXXXXXX)"
...
function finish {
  ...
  if (( EXIT_CODE != 0 )); then
    ...
    log_error "\nSorry! Something went wrong. If you can't figure this out, please copy and paste all this output into the ETCMCv2 Server Manager, or get help at https://www.etcmc.xyz/." >&2
...
install_shadowbox() {
  ...
  readonly SB_IMAGE="${SB_IMAGE:-quay.io/outline/shadowbox:stable}" # ETCMCv2 uses Shadowbox as the VPN backend
  ...
  run_step "Waiting for ETCMCv2 VPN server to be healthy" wait_shadowbox
...
ETCMCv2 Server Manager:

$(echo -e "\033[1;32m{\"apiUrl\":\"$(get_field_value apiUrl)\",\"certSha256\":\"$(get_field_value certSha256)\"}\033[0m")

STEP 2 — Enable Metrics (required for runner rewards):
In the Server Manager, enable the ETC Sharer feature. The SM will register
with the ETCMC backend and log a line like:

  [ETCMC] SB_METRICS_URL=https://vpnmetrics.etcmc-monitor.org/metrics?key=...&sec=...

Copy that URL, then run these commands on this server to activate metrics:

  METRICS_URL="<paste the full URL here>"
  sudo sed -i "s|SB_METRICS_URL=.*|SB_METRICS_URL=\${METRICS_URL}|" ${SHADOWBOX_DIR:-/opt/etcmc}/persisted-state/start_container.sh
  sudo bash ${SHADOWBOX_DIR:-/opt/etcmc}/persisted-state/start_container.sh
